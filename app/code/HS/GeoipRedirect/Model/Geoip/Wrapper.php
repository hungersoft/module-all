...
class Wrapper
{
    ...
    {
        $country_id = $this->getCountryIdByAddr($addr);
        return $country_id !== false ? $this->GEOIP_COUNTRY_CODES[$country_id] : false;
    }

    public function getCountryIdByAddr($addr) 
    {
        $ipnum = ip2long($addr);
        return $this->_seekCountry($ipnum) - $this->GEOIP_COUNTRY_BEGIN;
    }

    private function _seekCountry($ipnum) 
    {
        $offset = 0;
